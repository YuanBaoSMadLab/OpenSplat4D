...
#include "OpenSplat4DPointCloud.h"   // for the LogOpenSplat4D log category
...
// Logging
// ----------------------------------------------------------------------------
// [ENHANCEMENT vs. reference] The reference routed logs to printf() (no UE
// integration). We route them through the plugin's LogOpenSplat4D category so
// SPZ errors/warnings show up in the Output Log like the rest of OpenSplat4D.
#if defined(SPZ_ENABLE_LOGS)
#define SpzLog(Format, ...) UE_LOG(LogOpenSplat4D, Log, TEXT(Format), ##__VA_ARGS__)
#else
#define SpzLog(Format, ...)
#endif
...
namespace Spz {
	...
	namespace {
		...
		// [ENHANCEMENT] Bit-cast helpers that are UE-correct (FPlatformMemory
		// avoids the strict-aliasing UB of a raw reinterpret_cast, and works on
		// every platform UE supports). The reference used a bare BitCast<> which
		// is not always available outside its own build setup.
		uint32_t ReadU32(float f) { return FPlatformMemory::ReadUnaligned<uint32_t>(&f); }
		float WriteF32(uint32_t u) { float f; FPlatformMemory::WriteUnaligned<uint32_t>(&f, u); return f; }
		...
		int dimForDegree(int degree) {
			...
			case 3:
				return 15;
			default:
				SpzLog("[SPZ: ERROR] Unsupported SH degree: %d", degree);
				return 0;
			}
		}
...
#define CHECK_EQ(x, y) CHECK((x) == (y));
		...
		// --------------------------------------------------------------------
		// [ENHANCEMENT] OpenSplat4D 4DGS temporal extension block.
		// Appended right after the standard Niantic SPZ stream. A plain Niantic
		// decoder stops at the end of the packed gaussians and never touches
		// these bytes, so the layout stays byte-compatible with the reference.
		// --------------------------------------------------------------------
		constexpr uint8 GOPEN_SPZ_4D_MAGIC[4] = { 'O', '4', 'D', '4' };

		void serialize4DExtra(const TArray<FOpenSplat4DPoint>& g, std::ostream& out) {
			out.write(reinterpret_cast<const char*>(GOPEN_SPZ_4D_MAGIC), 4);
			uint32_t n = static_cast<uint32_t>(g.Num());
			out.write(reinterpret_cast<const char*>(&n), sizeof(n));
			for (const FOpenSplat4DPoint& p : g) {
				float at = p.AnchorTime;
				float tv = p.TimeVariance;
				out.write(reinterpret_cast<const char*>(&at), sizeof(at));
				out.write(reinterpret_cast<const char*>(&tv), sizeof(tv));
				float v[3] = { p.Velocity.X, p.Velocity.Y, p.Velocity.Z };
				out.write(reinterpret_cast<const char*>(v), sizeof(v));
				uint8_t b = p.bUseVelocity ? 1 : 0;
				out.write(reinterpret_cast<const char*>(&b), sizeof(b));
			}
		}

		// Returns true if a 4D block was found and applied; false (non-fatal) otherwise.
		bool deserialize4DExtra(std::istream& in, TArray<FOpenSplat4DPoint>& g) {
			uint8_t magic[4] = { 0, 0, 0, 0 };
			in.read(reinterpret_cast<char*>(magic), 4);
			if (!(magic[0] == 'O' && magic[1] == '4' && magic[2] == 'D' && magic[3] == '4')) {
				// No 4D block (e.g. a standard Niantic .spz file) -> keep 3DGS defaults.
				return false;
			}
			uint32_t n = 0;
			in.read(reinterpret_cast<char*>(&n), sizeof(n));
			if (n != static_cast<uint32_t>(g.Num())) {
				SpzLog("[SPZ: ERROR] 4D block point count (%u) != gaussian count (%d)", n, g.Num());
				return false;
			}
			for (FOpenSplat4DPoint& p : g) {
				float at = 0.f, tv = 1e10f;
				float v[3] = { 0.f, 0.f, 0.f };
				uint8_t b = 0;
				in.read(reinterpret_cast<char*>(&at), sizeof(at));
				in.read(reinterpret_cast<char*>(&tv), sizeof(tv));
				in.read(reinterpret_cast<char*>(v), sizeof(v));
				in.read(reinterpret_cast<char*>(&b), sizeof(b));
				p.AnchorTime = at;
				p.TimeVariance = tv;
				p.Velocity = FVector3f(v[0], v[1], v[2]);
				p.bUseVelocity = b != 0;
			}
			return true;
		}
	...
	PackedGaussians packGaussians(const TArray<FOpenSplat4DPoint>& g) {
	...
	TArray<FOpenSplat4DPoint> unpackGaussians(const PackedGaussians& packed) {
	...
	// ----------------------------------------------------------------------------
	// compress / decompress (the public OpenSplat4D entry points)
	// ----------------------------------------------------------------------------
	bool compress(const TArray<FOpenSplat4DPoint> g, int compressionLevel,
		int workers, std::vector<uint8_t>& output, bool bInclude4D) {
		if (g.Num() == 0) {
			SpzLog("[SPZ: ERROR] Parsed TArray<FOpenSplat4DPoint> is empty.");
			return false;
		}
		...
		// [ENHANCEMENT] Only append the 4D block when the cloud actually carries
		// 4D data. A pure static 3DGS cloud therefore produces a byte-identical
		// Niantic SPZ stream (no size penalty, full interop).
		if (bInclude4D) {
			bool bHas4D = false;
			for (const FOpenSplat4DPoint& p : g) {
				if (p.AnchorTime != 0.f || p.TimeVariance != 1e10f ||
					p.bUseVelocity || !p.Velocity.IsNearlyZero()) {
					bHas4D = true;
					break;
				}
			}
			if (bHas4D) {
				serialize4DExtra(g, ss);
			}
		}
		...
		if (!compressGzipped(reinterpret_cast<const uint8_t*>(uncompressed.data()), uncompressed.size(), &output)) {
			SpzLog("[SPZ: ERROR] gzip compression failed.");
			return false;
		}
	...
	bool decompress(const std::span<const uint8_t> input, TArray<FOpenSplat4DPoint>& output) {
		...
		// [ENHANCEMENT] Best-effort 4D extension read. If the stream ends right
		// after the packed gaussians (a standard Niantic .spz file), this simply
		// returns false and the temporal fields keep their safe 3DGS defaults.
		if (stream.good()) {
			deserialize4DExtra(stream, output);
		}
